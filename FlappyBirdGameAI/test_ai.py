...
Usage:
    python test_ai.py                    (with graphics)
    python test_ai.py --headless         (faster, no graphics)
"""
...
def test_ai(genome, config, headless=False):
    """
    ...
    Args:
        ...
        headless: If True, run without graphics (faster for testing)
    """
    ...
    if not headless:
        ...
        print("Running in HEADLESS mode (no graphics, faster)...")
    
    while run:
        clock.tick(30)
        
        # Only check events if not headless
        if not headless:
...
def main():
    # Check if --headless argument was passed
    ...
    test_ai(winner, config, headless=headless)
